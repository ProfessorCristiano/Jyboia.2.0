...
JYBOIA_FILES_STR = "Arquivos Jybóia"
PYTHON_FILES_STR = tr("Python files")
_dialog_filetypes = [
    (JYBOIA_FILES_STR, ".jy"),
    (PYTHON_FILES_STR, ".py .pyw .pyi .pyde .pyx .pye"),
    (tr("all files"), ".*"),
]

PYTHON_EXTENSIONS = {"jy", "py", "pyw", "pyi", "pyde"}
PYTHONLIKE_EXTENSIONS = {"jy", "pyx", "pyde", "toml"}
...
class Editor(BaseEditor):
    ...
    def write_local_file(self, target_path, content_bytes, save_copy):
        ...
        try:
            ...
            # Se for um arquivo .jy, gerar automaticamente o arquivo .py correspondente
            if target_path.endswith(".jy"):
                try:
                    from thonny.jyboia.transpiler import transpilar
                    codigo_jy = content_bytes.decode("utf-8", errors="replace")
                    codigo_py, _ = transpilar(codigo_jy, incluir_runtime=True, caminho_jy=target_path)
                    py_path = target_path[:-3] + ".py"
                    with open(py_path, "w", encoding="utf-8") as f_py:
                        f_py.write(codigo_py)
                except Exception as e:
                    logger.warning(f"Erro ao auto-gerar .py para {target_path}: {e}")
    ...
    def ask_new_local_path(self) -> Optional[str]:
        ...
        if type_var.get() == JYBOIA_FILES_STR or type_var.get() == "":
            new_filename = self._check_add_jy_extension(
                new_filename, without_asking=type_var.get() == JYBOIA_FILES_STR
            )
        elif type_var.get() == PYTHON_FILES_STR:
            new_filename = self._check_add_py_extension(
                new_filename, without_asking=True
            )
    ...
    def _check_add_jy_extension(self, path: str, without_asking: bool = False) -> str:
        assert path
        ...
        if "." not in name:
            if without_asking or messagebox.askyesno(
                ...
                message="Arquivos Jybóia utilizam a extensão .jy.\n\nDeseja salvar como '%s'?" % (name + ".jy"),
