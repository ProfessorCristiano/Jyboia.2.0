    ...
    def execute_current(self, command_name: str) -> None:
        ...
        if (
            ...
            if target_path.endswith(".jy"):
                try:
                    from thonny.jyboia.transpiler import transpilar_arquivo
                    py_target = target_path[:-3] + ".py"
                    transpilar_arquivo(target_path, py_target, incluir_runtime=True)
                    target_path = py_target
                except Exception as e:
                    logger.exception("Erro ao transpilar Jybóia antes de executar", exc_info=e)
