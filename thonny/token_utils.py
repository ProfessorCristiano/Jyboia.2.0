...
# Palavras-chave do Python e Jybóia (Português Estruturado)
try:
    from thonny.jyboia.keywords import PALAVRAS_CHAVE as _JY_KW, FUNCOES_EMBUTIDAS as _JY_BUILTINS
    _jy_kw_list = list(_JY_KW.keys())
    _jy_builtin_list = list(_JY_BUILTINS.keys())
except Exception:
    _jy_kw_list = []
    _jy_builtin_list = []
...
_all_keywords = list(set(keyword.kwlist + _jy_kw_list))
KEYWORD = matches_any("keyword", [r"\b" + k + r"\b" for k in _all_keywords] + _SOFT_KEYWORD)
_builtinlist = [
    str(name) for name in dir(builtins) if not name.startswith("_") and name not in keyword.kwlist
]

# Funções e tipos embutidos Jybóia
for b in _jy_builtin_list:
    if b not in _builtinlist and b not in _all_keywords:
        _builtinlist.append(b)
